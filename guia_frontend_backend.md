...
## Nuevos Endpoints (Frontend ↔ Backend)

> Todos requieren `empresa_id` (header `X-Empresa-Id` o query `?empresa_id=...`).

1) Dashboard – Tabla combinada

- Método: GET
- Ruta: `/dashboard/tabla`
- Query params opcionales: `estado`, `solicitante_id`, `limit` (def 50), `offset` (def 0)
- Ejemplo:
```javascript
const res = await fetch(`${API_BASE}/dashboard/tabla?empresa_id=${empresaId}&estado=abierta&limit=20&offset=0`)
const { ok, data, error } = await res.json()
```

2) Definiciones JSON por entidad/columna (listar)

- Método: GET
- Ruta: `/json/schema/<entity>/<json_field>`
- Ejemplo:
```javascript
const res = await fetch(`${API_BASE}/json/schema/solicitante/info_extra?empresa_id=${empresaId}`)
const { ok, data } = await res.json() // data: array de definiciones
```

3) Definiciones JSON agrupadas por entidad y columna

- Método: GET
- Ruta: `/json/schema`
- Respuesta: objeto `{ [entity]: { [json_column]: Definition[] } }`
- Ejemplo:
```javascript
const res = await fetch(`${API_BASE}/json/schema?empresa_id=${empresaId}`)
const { ok, data } = await res.json()
// data.solicitante.info_extra → definiciones de ese JSON
```

4) Upsert (crear/actualizar en lote) definiciones JSON
...
{
  "items": [
    {"key":"ocupacion","type":"string","required":true,"description":"Ocupación"},
    {"key":"ingresos","type":"number","required":false,"default_value":0}
  ]
}
```
- Ejemplo frontend:
```javascript
await fetch(`${API_BASE}/json/schema/solicitante/info_extra?empresa_id=${empresaId}` ,{
  method: 'POST',
...
5) Eliminar definiciones JSON

- Método: DELETE
- Ruta: `/json/schema/<entity>/<json_field>`
- Opcional: `key` (query o body). Si se envía, elimina solo esa definición; si no, elimina todas las definiciones de esa entidad/columna.
- Ejemplos:
```javascript
// Eliminar una clave específica
await fetch(`${API_BASE}/json/schema/solicitante/info_extra?empresa_id=${empresaId}&key=ocupacion`, { method: 'DELETE' })

// Eliminar todas las definiciones de esa columna
await fetch(`${API_BASE}/json/schema/solicitante/info_extra?empresa_id=${empresaId}`, { method: 'DELETE' })
```
